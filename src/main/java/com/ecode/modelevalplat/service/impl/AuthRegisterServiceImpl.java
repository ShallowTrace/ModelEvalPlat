...
public class AuthRegisterServiceImpl implements AuthRegisterService {
    ...
    private static final int MAX_REGISTER_USERS_PER_IP = 1000;

    // 分布式锁 key 前缀
    private static final String DISTRIBUTED_LOCK_PREFIX = "register:lock:";
    ...
    public ResVo<String> preRegister(HttpServletRequest httpServletRequest, String email) {
        try{
            ...
            String ipLoginKey = "ip:register:" + ipAddress;
            Long userCount = redisTemplate.opsForSet().size(ipLoginKey);
            if (userCount != null && userCount >= MAX_REGISTER_USERS_PER_IP) {
            ...
            log.error("预注册失败: {}", e.getMessage(), e);
    ...
    @CircuitBreaker(name = "completeRegister", fallbackMethod = "completeRegisterFallback")
    @Transactional(rollbackFor = Exception.class, isolation = Isolation.REPEATABLE_READ)
    @Override
    public ResVo<String> completeRegister(AuthCompleteRegisterDTO authCompleteRegisterDTO) {
        String email = authCompleteRegisterDTO.getEmail();
        String username = authCompleteRegisterDTO.getUsername();
        String password = authCompleteRegisterDTO.getPassword();
        String confirmPassword = authCompleteRegisterDTO.getConfirmPassword();
        String dynamicCode = authCompleteRegisterDTO.getDynamicCode();
        String uuid = authCompleteRegisterDTO.getUuid();
        ...
        try {
            ...
            // 3. 获取分布式锁
            lockValue = redisDistributedLock.acquireLock(lockKey, LOCK_EXPIRE_TIME);
            if (lockValue == null) {
                return ResVo.fail(StatusEnum.SYSTEM_BUSY);
            }
            ...
            if (lockValue != null && !redisDistributedLock.releaseLock(lockKey, lockValue)) {
                log.warn("释放注册分布式锁失败，lockKey: {}", lockKey);
            }
        }
    }

    public ResVo<String> completeRegisterFallback(AuthCompleteRegisterDTO authCompleteRegisterDTO, Throwable ex) {
        log.warn("completeRegister 降级触发，email: {}, 异常: {}", authCompleteRegisterDTO.getEmail(), ex.toString());
        return ResVo.fail(StatusEnum.SYSTEM_ABNORMALITY); // 可自定义枚举或使用通用错误码
    }
